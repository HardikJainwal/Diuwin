...
import { signupUser } from "../redux/Slice/signupSlice"; // Adjust path to your signup slice
import { Link } from "react-router-dom";
import { FaPhone, FaEnvelope, FaLock, FaKey, FaEye, FaEyeSlash } from "react-icons/fa";
import Header from "../components/Header";

function SignupPage() {
  const dispatch = useDispatch();
  const { loading, error, user } = useSelector((state) => state.signup); // Accessing Redux state
  ...
  const handleSubmit = (event) => {
    ...
    const signupData = isPhoneLogin
      ? { phoneNumber, email: "", password, inviteCode }
      : { email, phoneNumber: "", password, inviteCode };

    // Dispatching the signup action to the Redux store
    dispatch(signupUser(signupData)); // Triggering the API call through Redux
  };
    ...
    <div className="bg-custom-blue min-h-screen flex flex-col items-center justify-center">
      ...
      <div className="bg-gray-100 p-8 shadow-md w-full max-w-md h-full mt-10 flex flex-col justify-center">
        <div className="flex justify-center mb-4 gap-4">
          <button
            className={`flex flex-col items-center px-6 py-2 font-medium text-xl ${
              isPhoneLogin
                ? "text-custom-blue border-b-2 border-custom-pink"
                : " text-gray-400"
            }`}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6 mt-8">
          ...
          <div className="relative mb-4">
            <div className="flex mb-2 items-center text-md mt-4 font-medium text-gray-900 gap-1">
          ...
          <div className="flex items-start mb-4">
            ...
            <label
              ...
              className="ml-2 text-sm text-custom-blue"
            >
